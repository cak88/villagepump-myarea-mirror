...
python3 villagepump_myarea_mirror.py 2026/06/16 --publish --append --no-frame  # ガワは付けない（別のツールが持つ）
```
...
`--append` の挿入位置は、転記先の**末尾ブロックの上**、それが無ければ末尾。末尾ブロックとは
ナビ行と、その下に付くことがあるアイコン行（`[YYYY-MM.icon][YYYY-Www.icon]`）のこと。どちらも
ページの最後に居るガワなので、下に積むとチェーンや導線が本文に埋もれる。末尾から空行と
アイコンだけの行を飛ばして遡り、そこがナビ行なら採る——飛ばすのはこの2種類だけなので、本文の
途中に `←` `→` を含む行があっても誤爆しない。

### ガワを別のツールが持つとき（`--no-frame`）

`--no-frame` を付けると、日記ページでも上部2行（`第N週` / `％経過`）とナビ行を**付けない**。
転記元への参照リンクは付くので、`--append` の二重追記防止はそのまま効く。

転記先のページを別のツールが立てていて、そちらがガワを持つときに使う。`diarypage` が
`cak-diary` に毎日ガワ付きの日記ページを立てる運用がこれで、分担はこうなる:

| | 持ち主 |
| --- | --- |
| ページを立てる・上部2行・ナビ行 | `diarypage`（その日書いていなくても毎日立てる） |
| 日記の中身 | cak が直接書く |
| 井戸端に書いた分 | このツールが `--append --no-frame` で、直接書いた日記の下・ナビ行の上へ積む |

このツールは「その日 井戸端に書いていなければ何もしない」ので、ページの存在を保証する役には
向かない。だからガワの持ち主を `diarypage` 側に置いている。
